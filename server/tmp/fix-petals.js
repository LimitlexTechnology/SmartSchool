...
try {
    ...
    const petals = data.students.find(s => s.firstName.includes('Petals'));
    if (petals) {
        petals.classId = targetClassId;
        fs.writeFileSync(studentFile, JSON.stringify(data, null, 2));
        console.log('Successfully updated Petals\' classId.');
    } else {
        console.log('Petals not found in student list.');
    }
} catch (e) {
